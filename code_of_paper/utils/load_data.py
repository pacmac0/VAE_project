...
def load_static_mnist(args, **kwargs):
    ...
    val_loader = data_utils.DataLoader(
        validation, batch_size=args.test_batch_size, shuffle=False, **kwargs
    )
...
def load_dynamic_mnist(args, **kwargs):
    ...
    # preparing data
    x_train = train_loader.dataset.train_data.float().numpy() / 255.0
    x_train = np.reshape(
    ...
    x_test = test_loader.dataset.test_data.float().numpy() / 255.0
...
def load_caltech101silhouettes(args, **kwargs):
    ...
    caltech_raw = loadmat(
        os.path.join(
            "datasets", "Caltech101Silhouettes", "caltech101_silhouettes_28_split1.mat"
        )
    )
...
def load_freyfaces(args, TRAIN=1565, VAL=200, TEST=200, **kwargs):
    ...
    with open("datasets/Freyfaces/freyfaces.pkl", "rb") as f:
        ...
        # data = pickle.load(f)ccompatibility error, fix: add encoding

    data = (data[0] + 0.5) / 256.0
...
def load_cifar10(args, **kwargs):
    ...
    train_data = np.clip((training_dataset.train_data + 0.5) / 256.0, 0.0, 1.0)
    ...
    test_data = np.clip((test_dataset.test_data + 0.5) / 256.0, 0.0, 1.0)
...
def load_dataset(args, **kwargs):
    if args.dataset_name == "static_mnist":
        ...
        train_loader, val_loader, test_loader, args = load_caltech101silhouettes(
            args, **kwargs
        )
