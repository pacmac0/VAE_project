...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        ...
        evaluate_loss += loss.data.item()  # [0]
        evaluate_re += -RE.data.item()  # [0]
        evaluate_kl += KL.data.item()  # [0]
    ...
    if mode == "test":
        ...
        test_data, test_target = [
            torch.cat(test_data, 0),
            torch.cat(test_target, 0).squeeze(),
        ]
        ...
        if args.cuda:
            test_data, test_target, full_data = (
                test_data.cuda(),
                test_target.cuda(),
                full_data.cuda(),
            )
        ...
        # VISUALIZATION: plot real images
        plot_images(
            args, test_data.data.cpu().numpy()[0:25], dir, "real", size_x=5, size_y=5
        )
        ...
        plot_images(
            args, samples.data.cpu().numpy(), dir, "reconstructions", size_x=5, size_y=5
        )
        ...
        if args.prior == "vampprior":
            ...
            plot_images(
                args, pseudoinputs[0:25], dir, "pseudoinputs", size_x=5, size_y=5
            )
        ...
        t_ll_e = time.time()
        print(
            "Test lower-bound value {:.2f} in time: {:.2f}s".format(
        ...
        t_ll_e = time.time()
        print(
            "Train lower-bound value {:.2f} in time: {:.2f}s".format(
        ...
        t_ll_e = time.time()
        print(
            "Test log_likelihood value {:.2f} in time: {:.2f}s".format(
        ...
        # CALCULATE log-likelihood
        t_ll_s = time.time()
        log_likelihood_train = 0.0  # model.calculate_likelihood(full_data, dir, mode='train', S=args.S, MB=args.MB)) #commented because it takes too much time
        t_ll_e = time.time()
        print(
            "Train log_likelihood value {:.2f} in time: {:.2f}s".format(
