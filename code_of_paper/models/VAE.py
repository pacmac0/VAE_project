...
class VAE(Model):
    def __init__(self, args):
        ...
        self.q_z_logvar = NonLinear(
            300, self.args.z1_size, activation=nn.Hardtanh(min_val=-6.0, max_val=2.0)
        )

        # decoder: p(x | z)
        self.p_x_layers = nn.Sequential(
            GatedDense(self.args.z1_size, 300), GatedDense(300, 300)
        )
    ...
    def calculate_lower_bound(self, X_full, MB=100):
        ...
        for i in range(I):
            ...
            RE_all += RE.cpu().data.item()  # [0]
            KL_all += KL.cpu().data.item()  # [0]
            lower_bound += loss.cpu().data.item()  # [0]
