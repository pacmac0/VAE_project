...
class VAE(Model):
    def __init__(self, args):
        ...
        self.q_z2_layers = nn.Sequential(
            GatedDense(np.prod(self.args.input_size), 300), GatedDense(300, 300)
        )

        self.q_z2_mean = Linear(300, self.args.z2_size)
        self.q_z2_logvar = NonLinear(
            300, self.args.z2_size, activation=nn.Hardtanh(min_val=-6.0, max_val=2.0)
        )
        ...
        self.q_z1_layers_z2 = nn.Sequential(GatedDense(self.args.z2_size, 300))
        self.q_z1_layers_joint = nn.Sequential(GatedDense(2 * 300, 300))
        ...
        self.p_x_layers_joint = nn.Sequential(GatedDense(2 * 300, 300))

        if self.args.input_type == "binary":
            self.p_x_mean = NonLinear(
                300, np.prod(self.args.input_size), activation=nn.Sigmoid()
            )
        elif self.args.input_type == "gray" or self.args.input_type == "continuous":
            self.p_x_mean = NonLinear(
                300, np.prod(self.args.input_size), activation=nn.Sigmoid()
            )
            self.p_x_logvar = NonLinear(
                300,
